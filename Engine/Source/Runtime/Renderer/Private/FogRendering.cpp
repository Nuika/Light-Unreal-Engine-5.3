...
void SetupFogUniformParameters(FRDGBuilder& GraphBuilder, const FViewInfo& View, FFogUniformParameters& OutParameters)
{
	// Exponential Height Fog
	{
		...
		//Custom Goober
		OutParameters.DirectionalInscatteringLightDirection = (FVector3f)View.DirectionalInscatteringLightDirection;
		OutParameters.DirectionalInscatteringLightDirection.W = View.bUseDirectionalInscattering ? FMath::Max(0.f, View.DirectionalInscatteringStartDistance) : -1.f;
	...
	for(int32 ViewIndex = 0;ViewIndex < Views.Num();ViewIndex++)
	{
		...
		if(ShouldRenderFog(*View.Family))
		{
			if (Scene->ExponentialFogs.Num() > 0)
			{
				...
				// Custom Goober
				View.DirectionalInscatteringLightDirection = FVector(FogInfo.DirectionalInscatteringLightDirection);
